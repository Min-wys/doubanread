    ...
    <div class="lianzai-main" @mouseleave="changeshowid(-1)">
      <div v-for="item in list" :key="item.currentRank">
        ...
        <div class="ranking-big" v-show="item.currentRank === showid">
          ...
          <a class="touming">
            <img v-lazy="item.works.cover"  @click="toOneStory(item.works)" style="cursor: pointer" />
          </a>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    toOneStory(item) {
      this.$store.commit("REPLACE_BOOK", item);
      this.$store.commit("ISINBOOKSHELF", item.id);
